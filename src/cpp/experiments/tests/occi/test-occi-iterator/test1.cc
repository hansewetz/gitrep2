...
void select2(occi_auth const&auth){
  // sql statement, row and bind types, bind data
  typedef tuple<int>row_t;
  typedef tuple<string>bind_t;
  string sql{"select count(1) from mt_tmlo where usedintest is not null and matid=:1"};

  // read from input collection
  occi_source<row_t,bind_t>rows(auth,sql,10,bind_t("2313vl"));
