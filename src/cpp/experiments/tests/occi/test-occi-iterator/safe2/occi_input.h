...
public:
  // ctor, assign,dtor
  explicit occi_input_iterator(oracle::occi::Connection*conn,std::string const&sql,Bind const&bind=Bind{},std::size_t prefetchcount=0):
...
private:
  ...
  void nextrow(){
    ...
    if(fetcher_.getResultSet()->next())apply_with_index(fetcher_,currentRow_);
    else end_=true;
  }
  // state
...
public:
  ...
  explicit occi_source(occi_auth const&auth,std::string const&sql,Bind const&bind=Bind{},std::size_t prefetchcount=0):
  ...
  explicit occi_source(oracle::occi::Environment*env,occi_auth const&auth,std::string const&sql,Bind const&bind=Bind{},std::size_t prefetchcount=0):
  ...
  explicit occi_source(oracle::occi::Connection*conn,std::string const&sql,Bind const&bind=Bind{},std::size_t prefetchcount=0):
      conn_(conn),connpool_(nullptr),env_(nullptr),sql_(sql),bind_(bind),closeConn_(false),
      releaseConn_(false),terminateEnv_(false),prefetchcount_(prefetchcount){
  }
  // ctor taking a stateless connection pool
  explicit occi_source(oracle::occi::StatelessConnectionPool*connpool,std::string const&sql,Bind const&bind=Bind{},std::size_t prefetchcount=0):
  ...
  iterator begin()const{return iterator(conn_,sql_,bind_);}
