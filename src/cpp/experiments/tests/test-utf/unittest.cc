...
BOOST_AUTO_TEST_SUITE(utf8_tests)
  ...
  BOOST_AUTO_TEST_CASE(utf8_cp_encode_length){
    ...
    for(vector<range_len_t>::const_iterator rl=range_lengths.begin();rl!=range_lengths.end();++rl){
      cp_t start=rl->first.first;
      cp_t end=rl->first.second;
      size_t len=rl->second;
      for(cp_t cp=start;cp<end;++cp)
        BOOST_CHECK((unicode_function_traits<utf8_tag,detail::dummy_iterator>::cp_encode_length(cp)==len));
    }
  }
  ...
  BOOST_AUTO_TEST_CASE(utf8_encode_length){
    ...
    for(vector<range_len_t>::const_iterator it=cu_lengths.begin();it!=cu_lengths.end();++it){
      cu_t start=it->first.first;
      cu_t end=it->first.second;
      cu_t len=it->second;
      for(cu_t cu=start;cu<=end;++cu){
        size_t cu_len=0;
        BOOST_CHECK((unicode_function_traits<utf8_tag,detail::dummy_iterator>::cu_encode_length(cu,cu_len)==uni_error::no_error));
    ...
    for(size_t cu=0;cu<0x100;++cu){
      ...
      size_t cu_len;
      BOOST_CHECK((unicode_function_traits<utf8_tag,detail::dummy_iterator>::cu_encode_length(cu,cu_len)==uni_error::error_invalid_lead_byte));
    }
  }
  ...
  BOOST_AUTO_TEST_CASE(utf8_decode){
    ...
    uni_error::error_code err=unicode_function_traits<utf8_tag,iter_t>::decode(cus,cp,&end,&cu_len);
  ...
  BOOST_AUTO_TEST_CASE(utf8_encode){
    ...
    cu_t*cus_ptr=&cus[0];
    cp_t cp=0x20ac;
    uni_error::error_code err=unicode_function_traits<utf8_tag,cu_t*>::encode(cp,cus_ptr);
    BOOST_CHECK(err==uni_error::no_error);
