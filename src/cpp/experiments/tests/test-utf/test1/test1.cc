...
template<>struct get_max_encode_len<b2::utf8_tag>{
  ...
  void operator()(b2::cp_t cp){max_=std::max(max_,b2::unicode_cp_encode_length<b2::utf8_tag>(cp));}
  size_t max_;
};
...
int main(){
  ...
  typedef std::vector<char_t>cont_t;
  typedef std::vector<char_t>::iterator iter_t;
  ...
  it=find_if(ustart,uend,[](b2::cp_t cp){return cp<static_cast<b2::cp_t>(128);});
  if(it!=uend){
  ...
  auto encode_len=[](b2::cp_t cp){return b2::unicode_cp_encode_length<b2::utf8_tag>(cp);};
  size_t total_len=std::accumulate(ustart,uend,0,[&encode_len](std::size_t len,b2::cp_t cp){return len+encode_len(cp);});
  ...
  std::vector<b2::cp_t>cps;
  std::copy(ustart,uend,std::back_inserter(cps));
  std::vector<char_t>cus;
  ...
  std::vector<b2::cp_t>cps1(isstr.begin(),isstr.end());
  std::vector<char_t>cus1;
  std::copy(cps1.begin(),cps1.end(),b2::make_unicode_output_iterator<b2::utf8_tag>(std::back_inserter(cus1)));
  for(std::vector<char_t>::const_iterator it=cus1.begin();it!=cus1.end();++it){
  ...
  std::istreambuf_iterator<char>sstr_it_begin(sstr.rdbuf());
  std::istreambuf_iterator<char>sstr_it_end;
  auto uit_begin(b2::make_unicode_input_iterator<b2::utf8_tag>(sstr_it_begin));
  auto uit_end(b2::make_unicode_input_iterator<b2::utf8_tag>(sstr_it_end));
  std::for_each(uit_begin,uit_end,to_hex<b2::cp_t>(std::cout));
  std::cout<<std::endl;


// NOTE!
std::cout<<"(1) Use of input iterators to convert from cus to cps:"<<std::endl;
char const pin[]="\x68\x65\x6c\x6c\x6f\x20\x65\xcc\x81\x20\xf0\xaf\xa8\x9d\x20\x77\x6f\x72\x6c\x64";
auto uit_begin1(b2::make_unicode_input_iterator<b2::utf8_tag>(pin));
auto uit_end1(b2::make_unicode_input_iterator<b2::utf8_tag>(pin+std::strlen(pin)));
std::size_t count1=1;
for(auto uit=uit_begin1;uit!=uit_end1;++uit)std::cout<<std::dec<<count1++<<" ";
//std::for_each(uit_begin1,uit_end1,to_hex<b2::cp_t>(std::cout));
std::cout<<std::endl;
...
// NOTE! Does not work
  ...
  auto uit1_begin(b2::make_const_unicode_iterator<b2::utf8_tag>(str1.begin()));
  ...
  // test encoding with error code
  std::cout<<"Testing encoding with error code ..."<<std::endl;
  auto encoder12(b2::make_unicode_encode<b2::utf8_tag>(std::ostream_iterator<char_t>(std::cout)));
  b2::uni_error err12;
  encoder12(static_cast<b2::cp_t>(0xFDD0u),err12);
  std::cout<<std::endl<<"error: "<<err12<<std::endl;
}
