...
#ifndef __INDLIST_H__
#define __INDLIST_H__
#include <iostream>
#include <tuple>
#include <type_traits>
namespace tutils{
...
//template<typename IL>struct change_tuple_i;
...
// NOTE! Not yet done
/*
template<typename F,typename T,typename IL>struct foldl_tuple;
template<typename F,typename T,std::size_t i,std::size_t j,std::size_t...k>
struct foldl_tuple<F,T,indlist<i,j,k...>>{
  using IL=indlist<i,j,k...>;
  auto operator()(F f,T&&t,IL const&il=IL())->void{
    auto ct=change_tuple_i<typename firstn<2,IL>::type>();
    auto t12=ct(std::forward<T>(t));
    auto tail
    return foldl_tuple<F,std::tuple_cat(make_tuple(
  }
};
template<typename F,typename T,std::size_t i>
struct foldl_tuple<F,T,indlist<i>>{
  auto operator()(F f,T&&t)->T{
    return t;
  }
};
*/
// -----------------------------------------------------------------------------
// functions for transforming tuples
// -----------------------------------------------------------------------------
...
// create a new tuple by specifying a template taking an indlist
...
// pick elements from a set of tuples creating a new tuple
