...
// ctor
DummyTranslator::DummyTranslator(shared_ptr<TaskQueue>inq,shared_ptr<TaskQueue>outq):
    inq_(inq),outq_(outq){
}
// run task collector (moves tasks from task queue into job repository)
void DummyTranslator::operator()(){
...
ostream&operator<<(ostream&os,DummyTranslator const&dt){
