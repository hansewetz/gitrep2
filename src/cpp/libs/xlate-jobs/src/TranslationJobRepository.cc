...
shared_ptr<TranslationJob>TranslationJobRepository::getNextJob(){
  ...
  // remove from dleJobs_ and insert into startedJobs_
  shared_ptr<TranslationJob>job{newJobs_.front()};
  newJobs_.pop_front();
...
shared_ptr<TranslationJob>TranslationJobRepository::removeStartedJob(TranslationJobId const&jobid){
  ...
  THROW_RUNTIME("failed to find started job while trying to remove it, jobid: "<<jobid);
}
// print function
