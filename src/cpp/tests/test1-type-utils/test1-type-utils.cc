...
int main(int argc,char**argv){
  ...
  cout<<"xform pop_front template: "<<utils::transform_tuple<pop_front>(tu)<<endl;

  // transform a tuple using an indlist (pick elemnts based on an indlist)
  auto tv=make_tuple(1,"two",3,"four",5);
  using IL=indlist<0,4>;
  cout<<"xform using indlist: "<<utils::transform_tuple(IL(),tv)<<endl;

  // reverse a tuple
  auto tz=make_tuple(1,"two",3,"four",5);
  cout<<"reverse tuple1: "<<utils::reverse_tuple(tz)<<endl;
  cout<<"reverse tuple2: "<<utils::transform_tuple<utils::reverse>(tz)<<endl;
  ...
  cout<<"xform multiple tuples: "<<utils::transform_tuples(indlist<0,1,2>(),t1,t2,t3)<<endl;

  // apply a function to ith elemnt of a set of tuples
  auto u1=make_tuple(1,"one","ONE");
  auto u2=make_tuple(2,"two","TWO");
  cout<<"sum of first elemnt of several tuples: "<<utils::apply_tuple(std::plus<int>(),utils::transform_tuples<0>(u1,u2))<<endl;
  ...
  cout<<"transformed tuple: "<<utils::transform_tuple(type2string_helper(),d)<<endl;
  ...
  cout<<utils::transform_tuple(type2string_helper(),z2)<<endl;

  // rearange elements
  auto z3=make_tuple(0,1,2,3,4);
  cout<<"rearanged tuple: "<<utils::transform_tuple(indlist<4,0,2,3,2>(),z3)<<endl;

  // add tuple elemnts together
  auto z5=make_tuple(1,2.3,7);
  cout<<utils::apply_tuple(adder(),z5)<<endl;

  // reverse tuple elements
  auto ttu1=make_tuple(1,2,3,4,5);
  cout<<utils::transform_tuple<utils::reverse>(ttu1)<<endl;
  ...
  utils::apply_tuple_ntimes_with_indlist(utils::reverse<IL6>::type(),Print1Value(),z6);

  // transform each element in a tuple generating a new tuple
  auto z7=make_tuple(1,2,3,4);
  cout<<"mulyiply by 2: "<<utils::transform_tuple([](int val){return val*2;},z7)<<endl;
  ...
  cout<<utils::apply_nth_elem_tuples<1>(maketuple(),x1,x2,x3)<<endl;

  // call a function by reversing arguments
  using IL8=make_indlist_from_range<0,5>::type;
  cout<<"apply_with_indlist (reverse) ";
  utils::apply_with_indlist(utils::reverse<IL8>::type(),PrintNValues(),0,1,2,3,4,5);
  cout<<endl;
  ...
  cout<<utils::apply_nth_elem_tuples<1>(maketuple(),xx1,xx2,xx3)<<endl;

  // call a function for each tuple element passing (index, tuple-element) to function
  auto z8=make_tuple("one","two","thre");
  utils::apply_ind_tuple_ntimes_with_indlist(make_indlist_from_tuple<decltype(z8)>::type(),PrintNValues(),z8);
}
